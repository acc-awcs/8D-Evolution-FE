...
<h1 class="title">Join Collective Ending Point Poll</h1>
...
<div class="wrapper">
	<div class="option">
		...
		<svg
			class="qrcode"
			use:qr={{
				data: `${PUBLIC_BASE_URL}/poll/${data.group.endPollCode}`
			}}
		/>
	</div>
	<div class="or">OR</div>
	<div class="option visit">
		...
		<p>and enter code</p>
		<p class="code">{data.group.endPollCode}</p>
	</div>
</div>
...
<PresentationFooter
	num={data.group.endPollReadyParticipants.length}
	numLabel="Ready"
	onNext={data.group.endPollReadyParticipants.length > 0 ? callActionFromScript : null}
	onPrev={null}
/>
