...
<Modal handleClose={onClose}>
	<div class="header">
		<h1 class="title small">Export Surveys</h1>
		...
		use:enhance={() => {
			...
			return async ({ update, result }) => {
				...
				if (result?.data?.success) {
					...
					exportAndDownloadCsv(rows, 'surveys.csv');
					generated = true;
				}
			};
		}}
	>
		{#if !generated}
			<p>Generate an export of all survey data. Your export will be automatically downloaded.</p>
			<div class="buttons">
