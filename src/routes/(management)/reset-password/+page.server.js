		...
		try {
				...
				template_content: [
					{
						name: 'content',
						content: `
                <p>Reset your password! This link will expire in one hour.</p>
                <p><a href="${PUBLIC_BASE_URL}/reset-password/${body.token}" style="color: #303326; text-decoration: underline">Reset password</a></p>
                `
					}
				],
				message: {
			...
			if (Array.isArray(mailchimpResp)) {
				...
				if (result.status === 'sent') {
					...
					redirect(303, `/reset-password/sent`);
				} else {
					return fail(422, {
						success: false,
